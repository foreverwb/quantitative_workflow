...
def get_user_prompt(agent5_result: dict, calc_result: dict, agent3_data: dict) -> str:
    """用户提示词"""
    scenario_content = agent5_result.get("content", "{}")
    calc_json = calc_result.get("result", "{}")
    
    # 提取关键数据
    current_price = agent3_data.get("market_data", {}).get("current_price", 0)
    iv_percentile = agent3_data.get("volatility_analysis", {}).get("iv_percentile", 50)
